...
export function* checkAuthValidationSaga(action) {
...
export function* authUserSaga(action) {
    ...
    try {
        ...
        yield put(actions.checkAuthValidation(response.data.expiresIn));
    } catch(error) {
        yield put(actions.authFail(error.response.data.error));
    }
}

export function* authCheckStatusSage(action) {
    ...
    if (!token) {
        ...
        if (expirationDate <= new Date()) {
            ...
            yield put(actions.checkAuthValidation((expirationDate.getTime() - new Date().getTime()) / 1000));
        }
    }
}